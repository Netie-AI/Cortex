...
def validate_sql(sql: str, semantic: dict[str, Any]) -> GuardrailResult:
    ...
    for col in stmt.find_all(exp.Column):
        if col.name:
            ...
            if col.name not in _allowed_columns(semantic, "inventory") and col.name != "*":
                violations.append(f"UNKNOWN_COLUMN:{col.name}")

    star = stmt.find(exp.Star)
    if star and sensitive:
        cols = [c for c in _allowed_columns(semantic, "inventory") if c not in sensitive]
        stmt.set("expressions", [exp.column(c) for c in cols])
