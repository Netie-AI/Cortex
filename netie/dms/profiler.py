...
def profile_dataset(path: Path | str) -> ProfileReport:
    ...
    if "location" in col_values:
        locs = [v.strip() for v in col_values["location"] if v.strip()]
        canonical = {"wh-a": "WH-A", "wh-b": "WH-B"}
        ...
        for canon, variants in buckets.items():
            if len(variants) > 1:
                report.detected_issues.append(
                    DetectedIssue(
                        "location",
    ...
    if "last_updated" in col_values:
        formats = Counter()
        for v in col_values["last_updated"]:
            if not v.strip():
                continue
    ...
    if "sku" in col_values and "location" in col_values:
        seen: Counter[str] = Counter()
        for row in rows:
            sku = re.sub(r"[\s-]", "", str(row.get("sku", "")).upper())
            loc = re.sub(r"[\s_]", "", str(row.get("location", "")).lower())
            seen[f"{sku}|{loc}"] += 1
        dup_count = sum(c - 1 for c in seen.values() if c > 1)
        if dup_count:
            report.detected_issues.append(
                DetectedIssue("sku+location", "DUPLICATES", list(seen.keys())[:3], dup_count)
            )

    # Type chaos on reorder_level
    if "reorder_level" in col_values:
        types = Counter()
        for v in col_values["reorder_level"]:
            if not v.strip():
                types["null"] += 1
