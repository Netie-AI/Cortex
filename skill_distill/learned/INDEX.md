...
- (high/observed → rule) Cursor sidebar/cloud runs are session-based; they are not a background state loop
- (high/docs → rule) Cortex (DAG/routines/seeker/A2A) is the orchestration layer; do not add LangGraph as a second orchestrator
- (high/observed → rule) New task -> new Cursor chat; normal chat -> chatbot workspace
