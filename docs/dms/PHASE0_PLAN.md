# Phase 0 — Production Deploy Plan
...
- `.\demo\run_demo.ps1` → API :8000 + UI :3000 green
- `.\scripts\verify_all.ps1` → 19/19 checks (or documented skips only when demo not running)
...
| F7 remainder (SOPS, rate limit) | Not blocking demo | Parallel track — confirm before external pilot |
...
| Item | State |
|---|---|
| PII `redact_for_prompt` choke-point | Shipped — Gate F7 PASS |
| AES-256-GCM envelope crypto | Shipped — tests green |
| RLS SQL migrations | Shipped — not CI-verified without Postgres |
| SOPS+age secrets | **Debt** — env vars still |
| Rate limiting | **Debt** — not on API routes |
...
Captured skills feed **F4 suggest ranking only**. They must **never** modify F5 compliance YAML rules or gate thresholds. Any proposal to let skills change gate logic → `PARKING_LOT.md` P14 + separate gate.
...
### P0.1 — Env contract
- [ ] `demo/env.example` — `PACK`, `DMS_OPS_DB`, `DMS_LEDGER_DSN`, `DMS_SKILL_CAPTURE_ENABLED`, placeholders only
...
- [ ] Manual smoke: `/`, `/brain`, `/skills`, `/chat` — verdict colors + skills banner

### P0.5 — Gate P0 packet
- [ ] `docs/dms/GATE_P0_PACKET.md` — compose build, TLS curl, verify_all output, demo screenshot checklist
