...
- **Verify:** `python -m pytest tests/test_execution/test_legal_clamp.py -q -p no:cacheprovider` (166 tests) then `python -m pytest tests/ -q -p no:cacheprovider`
...
| Piece | Where |
|---|---|
| `JudgmentDecision.floor: str | None = None`, the name of the deterministic safety floor that produced the tier. `JudgmentModel.LEGAL_FLOOR`, `VIP_FLOOR`, `SAFETY_FLOORS`. `rules_decide` sets it on the legal branch and on the VIP bump; `decide()` sets it on the backend path when `apply_rules_floor` names a safety floor. The birthday quality floor and the T0 pin leave it `None`. | `CortexOS/routing/judgment_model.py` |
...
VERIFIED (this run, worktree on 275fe13):
