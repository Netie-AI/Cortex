...
- Assumed: the browser gate in CI (the tests use the same skip helper style as `test_session_grant_dialog.py`, so CI behaves identically to that file); that a founder's real UACC session has the page open on the space the agent runs in (the event is per space, and a page on another space does not see it).
