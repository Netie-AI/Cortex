...
- **Main idea:** CoT/route/improve for generative-ask lives in `CortexOS.crew.cot_climb` and calls the public FreeRoute adapter. Insights `generate=true` delegates to climb. GET `/crew/insights` stamps `cot_climb` INCOMPLETE. Reuses gen_cfsm G1 + dag_runner. JEPA stays proxy. Unarmed fail-closed. DMS #180 57.69%/38.46% WRONG=0 is cited, not replaced. FreeRoute G1-G6 stay on #215 @ 50267289. G4 litellm/brain/suggest not expanded (P24.1).
