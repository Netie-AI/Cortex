...
`GATE_F8_PACKET.md` already scopes almost exactly a Palantir-style action type, just without that name: a governed tool-call path with an allowlist (`skill_registry`'s `required_tools`), a compliance pre-check (`ComplianceEngine`), parameter sanitization (`injection_guard` + `pii.redact_for_prompt`), sandboxed execution (`WasmSandbox`), and a ledger event on completion.
...
| **O3 — Action-type registry wired into F8** | 1.5–2 days | **F7 remainder PASS** (existing STATUS.md rule: "F8 blocked until F7 remainder PASS") | This *is* F8 (per `docs/dms/GATE_F8_PACKET.md`), with `execute_tool_call_node()` resolving tools against `ontology_action_types` (§2.3) instead of a bespoke allowlist. First action type: `export_pptx`. Existing ledger event types (`item.intake`, `item.moved`, …) get registered, not renamed. | `CortexOS/execution/dag_runner.py`, `packs/dms/ontology/action_types.yaml`, `packs/dms/compliance/tool_call_rules_v1.yaml`, `tests/dms/test_tool_call.py` |
