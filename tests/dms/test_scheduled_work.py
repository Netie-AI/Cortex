...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CORTEX_SCHEDULED_BRIEF", "0")
...
def test_classify_email_and_ship_gate():
    ...
    assert "pr_create" not in scheduled_work.classify("code review open PRs every weekday")
    assert "review" in scheduled_work.classify("code review open PRs every weekday")
    assert "pr_create" not in scheduled_work.classify("Summarize my open PRs every weekday morning")
    assert "build" in scheduled_work.classify("Run the OpenForge 3d mesh ship-gate")
...
def test_ov_brief_prepends_without_dropping_facts():
    listed = {
        "ok": True,
        "prs": [{"number": 3, "title": "keep me", "url": "https://example.test/3"}],
        "law": "Do not auto-merge.",
    }
    out = scheduled_work.run(
        "Summarize my open PRs every weekday morning",
        list_prs_fn=lambda **k: listed,
        brief_fn=lambda facts: {"ok": True, "text": "One open PR titled keep me.", "error": ""},
    )
    assert out["output"].startswith("One open PR titled keep me.")
    assert "keep me" in out["output"]
    assert any(s["tool"] == "ov_brief" and s["ok"] for s in out["steps"])


def test_ov_brief_miss_keeps_operator_text():
    listed = {
        "ok": True,
        "prs": [{"number": 3, "title": "keep me", "url": "https://example.test/3"}],
    }
    out = scheduled_work.run(
        "Summarize my open PRs every weekday morning",
        list_prs_fn=lambda **k: listed,
        brief_fn=lambda facts: {"ok": False, "text": "", "error": "openvault_down"},
    )
    assert "keep me" in out["output"]
    assert not out["output"].startswith("One open")
    assert any(s["tool"] == "ov_brief" and not s["ok"] for s in out["steps"])
...
async def test_pr_routine_dispatch_uses_github(monkeypatch):
    listed = {
        "ok": True,
        "prs": [{"number": 9, "title": "real work", "url": "https://x/9"}],
        "law": "Do not auto-merge.",
    }
    monkeypatch.setattr(
