...
4. **Spend doors on Cortex's own credential.** Engine `POST /dms/query` and Crew `POST /crew/spaces/{id}/messages` (the agent loop) spend under Cortex's credential for any unauthenticated local caller; only `POST /crew/freeroute` and Insights generate apply caller authority. **Condition:** #213 lands API auth.
