...
_PR_CREATE = re.compile(
    r"\b(?:create|file|cut)\b.{0,24}\b(?:pr|pull request)s?\b"
    r"|\bopen\s+(?:a|the)\s+(?:pr|pull request)\b"
    r"|\bgh pr create\b",
    re.I,
)
_GITHUB = re.compile(
...
BriefFn = Callable[[str], dict[str, Any]]
...
def send_digest(subject: str, body: str) -> dict[str, Any]:
    ...
    password = (
        os.environ.get("SMTP_PASS") or os.environ.get("GMAIL_APP_PASSWORD") or ""
    ).strip().replace(" ", "")
...
def _ov_brief(facts: str) -> dict[str, Any]:
    """One FreeRoute completion over operator facts. Never invents the facts.

    Keys stay in OpenVault. `model=auto` is the Qwen/Deepseek/free pool.
    A down vault or HTTP miss leaves the deterministic gh/ship-gate/mail text.
    """
    text = (facts or "").strip()
    if not text:
        return {"ok": False, "error": "empty_facts", "text": ""}
    if os.environ.get("CORTEX_SCHEDULED_BRIEF", "1").strip().lower() in ("0", "false", "off"):
        return {"ok": False, "error": "brief_off", "text": ""}
    try:
        import httpx

        from CortexOS.execution.workflow_openvault import (
        ...
        from CortexOS.routing.adapters.openvault import resolved_openvault_token
    except Exception as exc:
        return {"ok": False, "error": type(exc).__name__, "text": ""}
    if not ov_ping():
        return {"ok": False, "error": "openvault_down", "text": ""}
    ensure_provider_keys()
    budget = check_openfree_budget(
        "scheduled_work",
    ...
    if not budget.get("ok"):
        return {"ok": False, "error": str(budget.get("error") or "freeroute_budget")[:200], "text": ""}
    headers = {"Content-Type": "application/json", "Accept": "application/json"}
    token = resolved_openvault_token()
    if token:
        headers["Authorization"] = f"Bearer {token}"
    url = f"{openvault_base_url().rstrip('/')}/v1/chat/completions"
    payload = {
        "model": "auto",
        "messages": [
            {
                "role": "system",
                "content": (
                    "Summarize these operator results for the host. "
                    "Do not invent pull requests, builds, or mail. "
    ...
    if resp.status_code != 200:
        kind = f"ov_http_{resp.status_code}"
        try:
            blob = resp.json() if resp.content else {}
            err = blob.get("error") if isinstance(blob, dict) else {}
            if isinstance(err, dict):
                kind = str(err.get("type") or err.get("message") or kind)[:80]
            elif isinstance(err, str) and err:
                kind = err[:80]
        except Exception:
            pass
        return {"ok": False, "error": kind, "text": ""}
    try:
        raw = resp.json() if resp.content else {}
    except Exception:
        return {"ok": False, "error": "ov_bad_json", "text": ""}
    ...
    if not content:
        return {"ok": False, "error": "ov_empty", "text": ""}
    return {"ok": True, "error": "", "text": content[:4000]}
    ...
    *,
    ...
    if kinds & {"github", "review", "pr_create"}:
        if list_prs_fn is None:
            ...
            # Chat desk keeps GH_WAIT_S at 1.5s. A scheduled sweep across CLAIMS
            # repos needs a longer gh timeout or every run is TimeoutExpired.
            def _list_prs(**kwargs):
                return github_mod.list_prs(timeout=20.0, **kwargs)

            list_prs_fn = _list_prs
        listed = list_prs_fn()
        ...
        if "review" in kinds:
            if pr_diff_fn is None:
                ...
                def _pr_diff(**kwargs):
                    return github_mod.pr_diff(timeout=20.0, **kwargs)
    ...
    if "email" in kinds:
        if inbox_fn is None:
            ...
            def _fetch_mail(limit: int = 8, **kwargs):
                return inbox_mod.fetch(limit=limit, timeout=20.0, **kwargs)

            inbox_fn = _fetch_mail
    ...
    if output:
        briefer = brief_fn if brief_fn is not None else _ov_brief
        brief = briefer(output)
        err = str(brief.get("error") or "")
        if brief.get("ok") and str(brief.get("text") or "").strip():
            steps.append(_step("ov_brief", True, "freeroute auto"))
            output = str(brief["text"]).strip() + "\n\n---\n" + output
        elif err and err != "brief_off":
            steps.append(_step("ov_brief", False, err[:240]))
    return {
        "ok": bool(ok),
