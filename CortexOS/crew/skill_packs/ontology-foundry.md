Ontology-foundry: one company model. Objects, links, actions. Palantir-shaped sketch, not AIP parity.

When: company ontology, object/link/action types, foundry path, Constructor connector -> ontology -> insight -> foundry -> app.
...
HITL: mutating writes, merges, live clicks, and pack publishes wait for operator confirm. Approvals stay manual. Do not auto-approve. Do not start or kill :8020.

Analog, not clone: OpenForge/AnalogCrawler are labs. Ideas (one schema, actions-only write, ontology as memory) stay Netie-voiced. Do not paste vendor strings.

Law: Cortex is the engine. Crew is a skin. Human is money and decision.
