"""Ontology registry loader + SQLite compiler (O1).
...
into the SAME ops DB the F1 ledger lives in (``data/dms_ops.db`` under the
packs root, or ``DMS_OPS_DB``) so registry rows are joinable against ledger
events. Plain functions + stdlib sqlite3, same style as packs/dms/audit/ledger.py.

Metadata is data, not code: the registry is rows describing objects/actions,
which is what lets one engine serve N verticals (see PALANTIR_AIP_RESEARCH.md §7).
"""
...
def _ontology_dir(pack_dir: Path | str | None = None) -> Path:
    base = Path(pack_dir) if pack_dir is not None else PACK_DIR
