...
Append-only history for Cortex. **Newest first.**

`STATUS.md` answers "what is true right now" and is capped at 60 lines; this
file is where its entries retire to. Nothing here is edited after the fact -
if an entry turns out to be wrong, add a newer one saying so rather than
rewriting the record.

Moved here on 2026-08-06, when STATUS.md had grown to 523 lines against that
cap and the current state was buried under nine months of history (R-0013 /
CLAUDE.md file law: five files per repo, STATUS.md <= 60 lines, CHANGELOG.md
append-only).
...
## Shipped layers at a glance (moved from STATUS.md 2026-08-06)
